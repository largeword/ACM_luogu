...
void copy1(char input[], char output[], int begin, int end)//传入一个未经处理的数组和一个空数组记录处理后的内容，传入开始元素位置和结束元素位置,处理开头不能为0的数字
{
...
void copy2(char input[], char output[], int begin, int end)//传入一个未经处理的数组和一个空数组记录处理后的内容，传入开始元素位置和结束元素位置,处理开头能为0的数字
{
	int copy_i, copy_j = end;
...
void reverse(char input[], char output[], int param)//根据输入的数组及数组类型倒转数组
{
	...
	{
		copy1(input, output, 0, reverse_location);
		output[reverse_location + 1] = '%';//末尾添加百分号
	}
	else if (reverse_sign == 2)//判断是否为小数
	{
		reverse_end = FindEnd(input);//找到最后一位数字的位置
		copy1(input, output, 0, reverse_location - 1);//倒转小数点前数字
		output[reverse_location] = '.';//添加小数点
		copy2(input, output, reverse_location + 1, reverse_end);//小数点之后的数可以从0开始
		for (reverse_i = reverse_end; reverse_i > reverse_location; reverse_i--)//小数点后的数字不能以0结尾
		{
	...
	{
		reverse_end = FindEnd(input);//找到最后一位数字的位置
		copy1(input, output, 0, reverse_location - 1);//倒转分号前数字
		output[reverse_location] = '/';//添加分号
		copy2(input, output, reverse_location + 1, reverse_end);//倒转分号后数字
	}
}
